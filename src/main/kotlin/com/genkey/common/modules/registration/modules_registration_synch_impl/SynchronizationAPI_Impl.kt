...
class SynchronizationAPI_Impl : AbstractModuleComp(),SynchronizationAPI
{
    ...
    override suspend fun sendDataToSpire()
    {
        ...
        if(totalCountResult is RepoResultState.Success)
        {
            ...
            {
                val subjectsResult = notSyncedSubjects.getSubjects(i * BATCH_SIZE, (i + 1) * BATCH_SIZE)
                if(subjectsResult is RepoResultState.Success)
                {
                    val subjectsList = subjectsResult.value
